...
app.post("/guess/:user/:number", (req, res) => {
  ...
  let responseObj = {
    code: GAME_CODES.ERROR,
    msg: req.language(LAN_KEY.NOT_STARTED)
  };
  ...
  if (pickedNumber) {
    if (isOngoing) {
      ...
      if (guess === pickedNumber) {
        ...
        responseObj = {
          code: GAME_CODES.LOWER,
          msg: req.language(LAN_KEY.LOWER)
        };
      } else {
        responseObj = {
          code: GAME_CODES.BIGGER,
          msg: req.language(LAN_KEY.BIGGER)
        };
      }
    } else {
